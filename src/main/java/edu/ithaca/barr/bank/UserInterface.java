            ...
            try{
                ...
                try{
                    ...
                    try{
                        ...
                        if (account == 0){
                            customer.transferSavingsAccount(amount, atm.getCustomers().get(transferId));
                        }
                        else if (account == 1){
                            customer.transferCheckingAccount(amount, atm.getCustomers().get(transferId));
                        }
                        else{
                            throw new Exception();
                        }
            ...
            if (account == 0){
                ...
                try{
                    ...
                    try{
                        int id = in.nextInt();
                        try{
                            Customer customer = atm.getCustomers().get(id);
                        ...
                        try{
                            ...
                            try{
                                ...
                                try{
                                    ...
                                    atm.createNewAccount(teller, customer, account, withdrawLimit, interestRate, startBal);
                                }
                                catch(Exception e){
                                    System.out.println("This customer does not exist, a new one will be made with the id: " + nextId);
                                    System.out.println("Enter a new password:");
                                    try{
                                        String password = in.next();
                                        atm.createNewAccount(teller, nextId, password, account, withdrawLimit, interestRate, startBal);
                                        nextId++;
                                    }
                                    catch(Exception e1){
                                        System.out.println("Invalid password");
            ...
            try{
                ...
                if (choice < 4){
                    ...
                    try{
                        ...
                        Customer customer = atm.getCustomers().get(customerId);
                        customerState(customer, choice);
                    }
                    catch(Exception e){
                        System.out.println("Invalid ID");
    ...
    private static void adminState(BankAdminSoftware admin){
        ...
        while(!done){
            ...
            try{
                ...
                if (choice == 0){
                    System.out.println(admin.sumAllAccounts(atm.getAccounts()));
                }
                else{
                    done = true;
                }
